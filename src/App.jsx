...
import {
  ...
  CheckCircle, BookOpen, Download,
  AlertCircle, Loader2, FileSpreadsheet, Code
} from 'lucide-react';
...
// Replace with your REAL keys for your local project
...
export default function SimpleDataCollector() {
  ...
  // SIMPLIFIED STATE: Removed unrelated, question, and keywords
  const [formData, setFormData] = useState({
    grade: 'Grade 1',
    related: '' // This is your Correct Sentence
  });
  ...
  // --- 5. Export Logic (Simpler now) ---

  const handleExportJSON = () => {
    // Structure: Image -> Sentence
    const exportData = entries.map(e => ({
      input: {
        image_context: "Image provided separately",
        grade_level: e.grade
      },
      output: {
        sentence: e.related
  ...
  const handleExportCSV = () => {
    ...
    const rows = entries.map(e => [
      ...
      e.grade,
      `"${e.related}"`,
      `"${e.imageUrl ? e.imageUrl.substring(0, 50) + '...' : 'No Image'}"`
    ]);
